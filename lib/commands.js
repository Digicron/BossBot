...
class Commands {
   ...
   */
    ...
    sendExpireMessage(msg, id) {
        ...
        this.bot.sendMessage(id, 'Momentan nu editezi nici un grup. Trimite `/setting` in grup pentru configurarea grupului.', { parse_mode: 'markdown' });
    }

    async settingCommand(msg) {
        ...
        if (msg.chat.type === 'supergroup') {
            ...
            if (this.commonFunctions.messageSenderIsAdmin(admins, msg)) {
                ...
                if (!cfg.allowAdminsToConfigure && (!this.commonFunctions.messageSenderIsChatOwner(admins, msg)) && allowedAdmins.indexOf(msg.from.id.toString()) == -1) {
                    this.bot.sendMessage(msg.from.id, '_Nu ai autorizatie pentru configurarea chatului_', {
                        parse_mode: 'markdown'
                    });
                    return;
                }
                ...
                if (!enoughtRights) {
                    ...
                    alertMsg = '_Botßul nu are destule drepturi in acest grup! Promoveazaßl ca admin, cu drepturi de \'stergere mesaje\' and \'ban useri\' !_';
    ...
    async kickByReply(msg) {
        ...
        if(!msg.reply_to_message){
            await this.bot.sendMessage(msg.from.id, 'Raspunde mesajului acelui utilizator pe care vrei sa il zbori din grup');
            return;
        }              
    ...
    async banByReply(msg) {
        ...
        if (!this.commonFunctions.messageSenderIsAdmin(admins, msg)) return;
        if(!msg.reply_to_message){
            await this.bot.sendMessage(msg.from.id, 'Raspunde mesajului acelui utilizator pe care vrei sa il banezi de pe grup');
            return;
        } 
        if (!this.commonFunctions.messageSenderIsAdmin(admins, msg.reply_to_message)) {
    ...
    async warnCommand(msg) {
        ...
        if (!this.commonFunctions.messageSenderIsAdmin(admins, msg.reply_to_message)) {
            ...
            this.bot.sendMessage(msg.chat.id, `${msg.reply_to_message.from.first_name} a fost avertizat de un admin. *${warns}/${maxWarn}*`, { parse_mode: 'markdown' });
            if (warns >= maxWarn) {
    ...
    async accessCommand(msg) {
        ...
        if (!this.commonFunctions.messageSenderIsChatOwner(admins, msg)) {
            this.bot.sendMessage(msg.from.id, 'Doar Ownerul poate seta accesul la configuratii');
            return;
        }
        let kbd = await this.getAccessKeyboard(admins, group);
        this.bot.sendMessage(msg.from.id, 'Configureaza adminii ce au acces la setarile botßului', {
            parse_mode: 'markdown',
            reply_markup: kbd
        });
    ...
    To configure bot in your group you need:
        ...
        3) Configure bot by sending /setting right into your group (message will disappear immediately).
    ...
    *Comenzi disponibile:*
    ...
    async actionHandler(groupId,action){
        if(action=='*resetBlacklist'){
            ...
            this.bot.sendMessage(groupId,'Lista cuvintelor interzise a fost actualizata de un admin');
        }
    }

    startCommand(msg) {
        this.log(this.actionTypes.start, msg);
        this.saveUser(msg,true);
        this.bot.sendMessage(msg.from.id, 'Well done! You can use /help command to get some documentation.');
    }

    // saves username and keep records
    async saveUser(msg,isAccesible) {
        if (msg.chat.type == 'private') {
            ...
            if (d && d._id) {
                await User.update({
                    ...
                    accessible: isAccesible
                });
            } else {
                let user = new User();
                user.userId = msg.chat.id;
                user.username = msg.chat.username;
                user.name = msg.chat.first_name + ' ' + msg.chat.last_name;
    ...
    // saves username and keep records from groups    
    async saveUserfromGroup(msg){
        if (msg.chat.type !== 'private') {
            let d = await User.findOne({
            ...
            if (d && d._id) {
                await User.update({
                    ...
                    $push: { groups : msg.chat.id.toString()}
                });
            } else {
                let user = new User();
                user.userId = msg.new_chat_member.id;
                user.username = msg.new_chat_member.username;                
                user.name = msg.new_chat_member.first_name + ' ' + msg.new_chat_member.last_name;
                user.accessible = false;
                user.groups=[];
                user.groups.push(msg.chat.id.toString());
                user.save(() => {});
            }
        }
    }
    ...
    async setHelloCommand(msg, match) {
        if (msg.chat.type !== 'private') {
            this.bot.sendMessage(msg.chat.id, 'use /setting menu to set hello command');
            return;
        }
    ...
    async getGroupThatUserCurrentlyConfigures(msg) {
        const currentlyEdit = await this.collections.mongoNowConfigatates
            .findOne({ user: msg.from.id, date: { $gte: this.commonFunctions.secondsAgo(63072000)} }).catch(console.dir);
